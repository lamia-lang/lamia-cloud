...
class TestConnectRepository:
    @patch("lamia_cloud.gcp.deployer._ensure_wif")
...
class TestWifBranchRestriction:
    ...
    @patch("lamia_cloud.gcp.deployer._ensure_per_repo_sa", return_value="sa@proj.iam")
    @patch("lamia_cloud.gcp.deployer._grant_sa_roles")
    @patch("lamia_cloud.gcp.deployer._get_project_number", return_value="123456")
    @patch("lamia_cloud.gcp.deployer.subprocess.run")
    def test_wif_condition_includes_ref_restriction(
        self, mock_run, mock_pn, mock_roles, mock_sa,
    ):
        mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
        from lamia_cloud.gcp.deployer import _ensure_wif
...
class TestIsRepositoryConnected:
    ...
    @patch("lamia_cloud.gcp.deployer.iam_admin_v1")
    @patch("lamia_cloud.gcp.deployer.subprocess.run")
